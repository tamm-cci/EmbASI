...
class SpinKpointArray:
    ...
    def __init__(
        ...
        # Create the XD Dictionary with array: (n_spin, n_kpoints, dim0, dim1, ...)
        #dtype = dtype or base_array.dtype
        self.data = base_array_dict

        self.ndim = base_array_dict[(0,0)].ndim + 2
    ...
    def __getitem__(self, key):
        ...
        for isp in range(spin_slice_min, spin_slice_max):
            for ikpt in range(kpt_slice_min, kpt_slice_max):
                if slice_all_check(key[2:]):
                    return self.data[(isp, ikpt)]
                else:
                    datkeys = [datkey for datkey in key[2:]]
                    return self.data[(isp, ikpt)][tuple(datkeys)]
            ...
            for isp in range(spin_slice_min, spin_slice_max):
                for ikpt in range(kpt_slice_min, kpt_slice_max):
                    if slice_all_check(key[2:]):
                        ...
                        datkeys = [datkey for datkey in key[2:]]
                        self.data[(isp, ikpt)][tuple(datkeys)] = value[(isp, ikpt)][tuple(datkeys)]
        else:
            for isp in range(spin_slice_min, spin_slice_max):
                for ikpt in range(kpt_slice_min, kpt_slice_max):
                    if slice_all_check(key[2:]):
                        ...
                        self.data[(isp, ikpt)][tuple(datkeys)] = value
...
def slice_all_check(slicefuncs):
    for slicefunc in slicefuncs:
        if (slicefunc.start is not None) or (slicefunc.stop is not None):
            return False
    return True
