...
def hamiltonian_eigensolv_parallel(hamiltonian, overlap, nelec):
    ...
    thresh = 1e-5
    n_basis = np.shape(overlap_dist)[0]
    xform_mat, n_bad = overlap_illcondition_check(overlap_dist, thresh)
    n_good = n_basis - n_bad

    evals, evecs = pdsyevx_from_numpy_array(xform_hamiltonian(hamiltonian, xform_mat), np.shape(hamiltonian)[0])
